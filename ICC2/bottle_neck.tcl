...
set nop [sizeof_collection [get_timing_paths -through $pn -max_paths 200 -slack_lesser_than 0]]
if {$nop > 1} {
set ref_nn [get_attribute [get_cell -of_object [get_pin $pn] ] ref_name ]
sh touch ./outputs/R/bottleneck.txt
set op [open "./outputs/R/bottleneck.txt" a]
if {[regexp -nocase {HVT| RVT} $ref_nn]} {
puts $op "$pn $nop $ref_nn"
}
close $op
}
}
